...
# --- CONFIGURATION ---
# Adjusted to MovieLens 32M data
CANDIDATES_PATH = RETRIEVED_DATA_PATH / 'movielens-32m/reduced_candidates_non_zero_recall.csv'
TRAIN_PATH = PROCESSED_DATA_PATH / 'movielens-32m_global_temporal_split/train.csv'
# Use raw movies.csv as metadata source
METADATA_PATH = RAW_DATA_PATH / 'movielens-32m/movies.csv'
OUTPUT_PATH = PROMPTING_DATA_PATH / 'movielens_prompting-data.jsonl'
...
def load_metadata_lookup(filepath):
    """
    Reads the MovieLens movies.csv and creates a dictionary mapping
    movie_id -> {title, desc}. Description is derived from genres string.
    Expected columns: movieId,title,genres (no header case handled below).
    """
    print(f"Loading metadata from {filepath}...")
    lookup = {}

    try:
        # movies.csv might have header; enforce names to be safe
        df = pd.read_csv(filepath, names=['id', 'title', 'genres'])
    except FileNotFoundError:
        print(f"Error: Metadata file not found at {filepath}")
        return {}

    for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing Metadata"):
        item_id = str(row['id']) if pd.notna(row['id']) else None
        if not item_id:
            continue
        ...
        # Genres may be pipe-separated or concatenated; replace '|' with ', '
        description = str(genres_raw).replace('|', ', ')
...
def get_user_histories(filepath):
    ...
    try:
        df = pd.read_csv(filepath, names=['user_id', 'item_id', 'rating', 'timestamp'], dtype={'user_id': str, 'item_id': str})
    except FileNotFoundError:
