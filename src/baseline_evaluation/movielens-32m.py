...
# Settings
model_path = config.MODEL_PATH / 'movielens-32m/LightGCN-movielens-32m_ver_0.4.pth'
test_file_path = config.PROCESSED_DATA_PATH / 'movielens-32m_global_temporal_split/test.csv'
output_csv_name = config.RETRIEVED_DATA_PATH / "movielens-32m/ver_0.4_movielens_candidates.csv"  # File to save for reranking
...
config['data_path'] = ATOMIC_DATA_PATH / 'movielens-32m'  # Ensure correct data path
...
for i in range(0, len(external_user_ids), BATCH_SIZE):
    ...
    for idx, ext_uid in enumerate(batch_ext_uids):
        ...
        str_ext_uid = str(ext_uid)
        if str_ext_uid in user_mapping:
            batch_int_uids.append(user_mapping[str_ext_uid])
